...
class UpgradeAlg:  # класс алгоритма муравьиной колонии для решения задачи коммивояжёра
    def __init__(self, func, numCity, size_pop, max_iter, distance_matrix=None, alpha=1, beta=3, rho=0.1):
        self.func = func
        ...
        self.Tau = np.ones((numCity, numCity))
    ...
    def run(self, max_iter=None):
        ...
        for i in range(len(self.prob_matrix_distance[0])):
            for k in range(len(self.prob_matrix_distance)):
                if self.prob_matrix_distance[i][k] != 0:
                    Emax += entropy(self.prob_matrix_distance[i][k], base=10)
        for i in range(self.max_iter):
            ...
            for j in range(self.size_pop):  # для каждого муравья
                ...
                self.Table[j, 0] = 0
                for k in range(self.numCity - 1):  # каждая вершина, которую проходят муравьи
