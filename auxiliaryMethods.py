...
from anotherAlg import SolveTSPUsingACO
from initCities import enterFile, num_point, cal_distance_matrix, cal_total_distance, column
...
def mainElit():
    _nodes = enterFile()
    alg = SolveTSPUsingACO(mode='Elitist', colony_size=100, steps=200, nodes=_nodes)
    best_x, best_y , global_best_tour= alg.run()
    fig, ax = plt.subplots(1, 2)
    ax[0].plot(best_x, best_y, 'o-r')
    pd.DataFrame(global_best_tour).cummin().plot(ax=ax[1])
    plt.rcParams['figure.figsize'] = [20, 10]
    plt.show()

def mainMinMax():
    _nodes = enterFile()
    alg = SolveTSPUsingACO(mode='MaxMin', colony_size=100, steps=200, nodes=_nodes)
    best_x, best_y , global_best_tour= alg.run()
    fig, ax = plt.subplots(1, 2)
    ax[0].plot(best_x, best_y, 'o-r')
    pd.DataFrame(global_best_tour).cummin().plot(ax=ax[1])
...
def mainMinMaxGraphAVGSpeedStudyRepOneLine():
    distance_matrix = cal_distance_matrix(enterFile())
    resCom = []
    resUpd = []
    arr = []
    ...
    for i in range(p):
        alg = SolveTSPUsingACO(mode='Elitist', colony_size=50, steps=50, nodes=_nodes)
        alg.run()
...
def mainComGraphAVGSpeedStudyRep():
    ...
    for i in range(2):
        print(i)
        alg = CommonAlg(func=cal_total_distance, numCity=num_point(enterFile()), size_pop=5, max_iter=5,
                        distance_matrix=distance_matrix)
        alg.run()

        alg2 = UpgradeAlg(func=cal_total_distance, numCity=num_point(enterFile()), size_pop=5, max_iter=5,
                          distance_matrix=distance_matrix)
        alg2.run()
...
def mainMinMaxGraphAVGSpeedStudyRep():
    distance_matrix = cal_distance_matrix(enterFile())
    resCom = []
    resUpd = []
    arr = []
    ...
    for i in range(30):
        print(i)
        alg = SolveTSPUsingACO(mode='MaxMin', colony_size=50, steps=50, nodes=_nodes)
        alg.run()
...
def mainElitGraphAVGSpeedStudyRep():
    distance_matrix = cal_distance_matrix(enterFile())
    resCom = []
    resUpd = []
    arr = []
...
def mainElitGraphAVGSpeedStudy():
    distance_matrix = cal_distance_matrix(enterFile())
    _nodes = enterFile()
    alg = SolveTSPUsingACO(mode='Elitist', colony_size=50, steps=50, nodes=_nodes)
    alg.run()
...
def mainMinMaxGraphAVGSpeedStudy():
    distance_matrix = cal_distance_matrix(enterFile())
    _nodes = enterFile()
    alg = SolveTSPUsingACO(mode='MaxMin', colony_size=50, steps=50, nodes=_nodes)
    alg.run()
...
def MinMaxRep():
    ...
    for i in range(20):
        ...
        x = alg.global_best_tour
        print(i)
        print(alg.global_best_tour)
        df = pd.DataFrame(x).cummin()
        q = []
...
def AllAvg():
    ...
    distance_matrix = cal_distance_matrix(enterFile())
    for i in range(30):
        Ualg = UpgradeAlg(func=cal_total_distance, numCity=num_point(enterFile()), size_pop=10, max_iter=10,
                          distance_matrix=distance_matrix)
        Ualg.run()

        Calg = CommonAlg(func=cal_total_distance, numCity=num_point(enterFile()), size_pop=10, max_iter=10,
                         distance_matrix=distance_matrix)
        Calg.run()
